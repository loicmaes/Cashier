...
import { type IEmail, useDefaultAttachements, useDefaultSubject, useDefaultTo } from "~/types/generics/email";
...
export async function sendMail(email: IEmail) {
  ...
  return useMailer().sendMail({
    ...
    options: {
      ...
      attachments: email.attachments ?? email.template.attachments ?? useDefaultAttachements(),
    },
  });
}
